...
import lecturerFakerFactory from "./lecturerFakerFactory";
...
const LecturerFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = lecturerFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("lecturer").create(elem));
...
export default connect(mapState, mapDispatch)(LecturerFakerDialogComponent);
