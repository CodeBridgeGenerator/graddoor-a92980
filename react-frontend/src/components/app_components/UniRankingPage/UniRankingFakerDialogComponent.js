...
import uniRankingFakerFactory from "./uniRankingFakerFactory";
...
const UniRankingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = uniRankingFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("uniRanking").create(elem));
...
export default connect(mapState, mapDispatch)(UniRankingFakerDialogComponent);
