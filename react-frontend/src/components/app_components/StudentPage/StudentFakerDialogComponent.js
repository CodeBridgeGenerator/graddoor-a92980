...
import studentFakerFactory from "./studentFakerFactory";
...
const StudentFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = studentFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("student").create(elem));
...
export default connect(mapState, mapDispatch)(StudentFakerDialogComponent);
