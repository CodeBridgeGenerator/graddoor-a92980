...
import uniReviewFakerFactory from "./uniReviewFakerFactory";
...
const UniReviewFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = uniReviewFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("uniReview").create(elem));
...
export default connect(mapState, mapDispatch)(UniReviewFakerDialogComponent);
