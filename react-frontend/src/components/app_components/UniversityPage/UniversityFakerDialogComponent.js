...
import universityFakerFactory from "./universityFakerFactory";
...
const UniversityFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = universityFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("university").create(elem));
...
export default connect(mapState, mapDispatch)(UniversityFakerDialogComponent);
