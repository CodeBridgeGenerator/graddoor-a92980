...
import reviewFakerFactory from "./reviewFakerFactory";
...
const ReviewFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = reviewFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("review").create(elem));
...
export default connect(mapState, mapDispatch)(ReviewFakerDialogComponent);
